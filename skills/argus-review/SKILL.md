...
4. **Run the mechanical pre-pass** (mechanical-checks.md) — cheap grep over the diff for mechanically-detectable bans → seed list of candidate hits with `file:line`, passed to the **conventions** reviewer as "pre-found candidates — verify each against the rule source, then extend."
