...
> Source: [@trq212 on X](https://x.com/trq212/status/2033949937936085378) — Anthropic internal learnings
...
- [Skilljar course on Agent Skills](https://anthropic.skilljar.com/introduction-to-agent-skills)
...
A skill is a folder, not just a markdown file. You should think of the entire file system as a form of context engineering and progressive disclosure. Tell Claude what files are in your skill, and it will read them at appropriate times.
...
When Claude Code starts a session, it builds a listing of every available skill with its description. This listing is what Claude scans to decide "is there a skill for this request?" Which means the description field is not a summary — it's a trigger signal.

**Keep it short** — every description is loaded into context for every session, regardless of whether the skill fires. Long descriptions waste token budget and dilute the signal across the full skill listing. Aim for 1-2 sentences max.

**Make it actionable** — describe the *situation* that should activate the skill, not the internal mechanics. Lead with "After…", "When…", or a condition the agent can pattern-match against the user's request.

**Good**: `After a backend API change (DTO, route, response shape), trace the impact through every downstream layer and fix mismatches.`
**Bad**: `Ensures frontend-backend contract integrity after API changes in a monorepo. Traces impact from backend (DTO, controller, route) through shared types, API client, TanStack hooks, and components — then fixes mismatches. This skill should be used after modifying a backend endpoint, DTO, response shape, route path, status code, or Zod/Prisma schema that feeds an API response. Also triggers when the user says "sync the frontend" or "update the hook" after a backend change.`
...
Data stored in the skill directory may be deleted when you upgrade the skill, so you should store this in a stable folder. As of today, `${CLAUDE_PLUGIN_DATA}` is provided as a stable folder per plugin to store data in.
...
- Make a **plugin** and have a Claude Code Plugin marketplace where users can upload and install plugins ([documentation](https://code.claude.com/docs/en/plugin-marketplaces))
...
To understand how a skill is doing, we use a PreToolUse hook that lets us log skill usage within the company ([example code](https://gist.github.com/ThariqS/24defad423d701746e23dc19aace4de5)). This means we can find skills that are popular or are undertriggering compared to our expectations.
